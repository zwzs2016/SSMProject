...
public class ServerEventListenerImpl implements ServerEventListener
{
	...
	private static final String LOGIN_SERVER="http://localhost:8888/tologin";
	/**
	 ...
	 * ，将立即调用回调方法 { #onUserLoginSucess(int, String, IoSession)}。
	...
	public int onUserLoginVerify(String userId, String token, String extra, Channel session)
	{
		...
		String url=LOGIN_SERVER;
		return new OkHttpUtil().sendPostByForm(url, map);
	}
	 ...
	 * <br>
	 * 注意：本回调在一种特殊情况下——即用户实际未退z出登陆但再次发起来登陆包时，回调也是一定会被调用。
	 * 
	 * @param userId 传递过来的准一id，保证唯一就可以通信，可能是登陆用户名、也可能是任意不重复的id等，具体意义由业务层决定
